...
The schema authority is the bundle YAML itself; this page mirrors the TypeScript definitions at `grounds-forge/src/bundle/types.ts`.
...
shared:
  nats:
    tailnet: nats.dev
    port: 4222
    description: NATS JetStream — Cross-Service-Events, KV-Store für Player-Presence
  postgres:
    tailnet: postgres.dev
    port: 5432
    description: Pro Engineer eine Database (`<handle>_<service>`), shared Server
  keycloak:
...
| Field | Type | Required | Notes |
|---|---|---|---|
| `<endpoint>.tailnet` | string | yes | MagicDNS hostname. Engineer needs the route enabled in their Tailscale ACL. |
...
- [Test Environment overview](/platform/test-environment) — engineer onboarding from `grounds login` to a hot-reload loop
- DevSpace template files: `groundsgg/library-platform-bundle/devspace/` (private repo)
- Schema source-of-truth: `grounds-forge/src/bundle/types.ts`
