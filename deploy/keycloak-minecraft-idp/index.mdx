...
<Note>
Grounds deployments can use the provider's managed Minecraft Java identity
attributes to resolve player identities for
[In-Game Permissions](/reference/plugins/in-game-permissions). The permission system
remains a separate authorization layer.
</Note>

If you are setting up the provider, start with the [installation guide](/deploy/keycloak-minecraft-idp/installation) and then continue with the [configuration guide](/deploy/keycloak-minecraft-idp/configuration).
...
<Card title="Installation" icon="download" href="/deploy/keycloak-minecraft-idp/installation">
  Install the provider JAR into Keycloak and configure the required Microsoft app registration.
</Card>

<Card title="Configuration" icon="sliders" href="/deploy/keycloak-minecraft-idp/configuration">
