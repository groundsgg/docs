...
installed. Resolve and use that service from the code that handles your player
action.

```kotlin
import gg.grounds.permissions.Permissions
import gg.grounds.permissions.minestom.GroundsPermissionsModule
import gg.grounds.runtime.GroundsServerContext
import java.util.UUID

fun installPermissions(ctx: GroundsServerContext) {
    GroundsPermissionsModule().install(ctx)
}
