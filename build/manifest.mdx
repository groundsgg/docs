...
### `flavors` (Minestom configuration variants)

Use `flavors` when one project publishes more than one Minestom runtime configuration. The key is selected with `grounds push --flavor <key>`; when `flavors` is present, selecting a key is required.

Keep the project `name` at the top level and put the complete Minestom runtime configuration in each flavor:
...
The selected flavor must define `type`, `baseImage`, `build.task`, `build.artifact`, and at least one module. Use `minestom-server` for the type; `minestom` is accepted as a shorthand. Do not combine top-level `plugins` entries with `flavors`, because local workspace resolution needs one source layout.
