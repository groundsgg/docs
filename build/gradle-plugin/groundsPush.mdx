...
Forge performs the final runtime image selection server-side from the [base image catalog](/build/concepts/base-images). The Gradle plugin sends the manifest source key, not an OCI tag or digest.
