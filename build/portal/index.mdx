---
title: "Portal"
description: "The web UI at portal.platform.grnds.io — projects, apps, members, Minecraft access, pushes, previews, platform access, and platform base images."
---
...
Developers reference source keys in `grounds.yaml`, for example `baseImage: paper`. Forge resolves the source to the stable catalog version during push. See [Base images](/build/concepts/base-images).

### Control Center

Users with `CONTROL_CENTER_VIEW` can open the **Platform Control Center** from the header. The first module is **Access**, which manages platform users, access roles, permissions, identity mappings, direct grants, and access audit events.

See [Control Center access](/build/portal/control-center-access) for the access-management workflow and [Control Center step-up](/build/portal/control-center-step-up) for protected mutation authentication.
